            ...
            public void run() {
                if (message.isFree()) {
                    if (!Waystones.getConfig().teleportButton || Waystones.getConfig().teleportButtonReturnOnly
                        || !PlayerWaystoneData.canFreeWarp(ctx.getServerHandler().playerEntity)) {
                        return;
                    }
                }
                if (WaystoneManager.teleportToWaystone(ctx.getServerHandler().playerEntity, message.getWaystone())) {
                    if (WaystoneManager.getServerWaystone(
                        ...
                        if (message.isFree()) {
                            PlayerWaystoneData
                                .setLastFreeWarp(ctx.getServerHandler().playerEntity, System.currentTimeMillis());
                        } else {
                            PlayerWaystoneData
                                .setLastWarpStoneUse(ctx.getServerHandler().playerEntity, System.currentTimeMillis());
                        }
                    }
                }
